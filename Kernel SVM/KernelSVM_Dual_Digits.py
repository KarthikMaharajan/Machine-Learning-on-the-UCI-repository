...
def test(minlambdaVal):
	...
	testFile = open('Data/optdigits.tes', 'rb')
	...
	testMatrix = numpy.delete(testPartition,64,1)
...
def validateVector(testVector,label):
	...
	for i in range(0,len(trainMatrix)):
		calculatedLabels = calculatedLabels+numpy.dot(alpha[i],computeKernelFunction(testVector,trainMatrix[i]))
