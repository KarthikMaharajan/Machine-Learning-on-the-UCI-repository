...
def dataImport():
	...
	for key,value in partitionsMap.iteritems():
		...
		for row in trainPartition:
			...
			trainPartitionLabels[p] = row[0]
			p = p+1
		p=0
		for row in testPartition:
			testPartitionLabels.append([])
			testPartitionLabels[p] = row[0]
			p = p+1
		#train on trainPartition
		computeKernelMatrix()
		for lambdaVal in frange(0,5,0.5):
			error = 0
			kernelSVM(lambdaVal)
			validate()
...
def test(minlambdaVal):
	...
	testFile = open('Data/testing.csv', 'rb')
	testData = list(csv.reader(testFile, delimiter = ','))

	#converting read features, labels to integers
	for i in range(0,len(testData)):
		testData[i] = list(map(float, testData[i]))

	error = 0
	kernelSVM(minlambdaVal)
	testMatrix = numpy.delete(testPartition,0,1)
...
def validate():
	...
	testMatrix = numpy.delete(testMatrix,0,1)
	for i in range(0,len(testMatrix)):
		validateVector(testMatrix[i],testPartitionLabels[i][0])
...
def validateVector(testVector,label):
	...
	trainMatrix = numpy.delete(trainPartition,0,1)
	testMatrix = numpy.delete(testPartition,0,1)
...
def computeKernelMatrix():
	...
	trainMatrix = numpy.delete(trainMatrix,0,1)
